...
import (
	...
	_ "github.com/spinframework/spin-go-sdk/v4/wit_component"
	wasiHandler "github.com/spinframework/spin-go-sdk/v4/wit_component/export_wasi_http_0_3_0_rc_2025_09_16_handler"
...
var wasiHandle = func(request *types.Request) wit_types.Result[*types.Response, types.ErrorCode] {
	...
	httpResp := NewHttpResponseWriter(tx)

	go func() {
		defer tx.Drop()

		// Call the user's handle
		handler(httpResp, httpReq)

		// Ensure reconcilliation
		_ = httpResp.reconcile()
	}()
	...
	return wit_types.Ok[*types.Response, types.ErrorCode](response)
}

// TODO: Not sure what this is...
func trailersFuture() *wit_types.FutureReader[wit_types.Result[wit_types.Option[*types.Fields], types.ErrorCode]] {
	tx, rx := types.MakeFutureResultOptionFieldsErrorCode()
	go tx.Write(wit_types.Ok[wit_types.Option[*types.Fields], types.ErrorCode](wit_types.None[*types.Fields]()))
	return rx
}
